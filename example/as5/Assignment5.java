...
public class Assignment5 {
	...
	private JTextArea ngramsOutput;
	...
	private JPanel renderNgrams() {
		...
		load.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				...
				if (fileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
					File file = fileChooser.getSelectedFile();
					ngramsOutput.setText(readFile(file));
				}
			}
		});
//		// put it all together.
...
//		// a Train Ngrams button.
		...
		JPanel ngramsLeft = new JPanel();
		layout = new BorderLayout();
		layout.setVgap(LAYOUT_GAP);
		ngramsLeft.setLayout(layout);
		ngramsLeft.add(new JPanel(), BorderLayout.CENTER);
		ngramsLeft.add(leftTop, BorderLayout.NORTH);
		ngramsLeft.add(leftBottom, BorderLayout.SOUTH);

		ngramsOutput = new JTextArea(COMP_OUTPUT_ROWS, COMP_OUTPUT_COLS);

		JPanel ngramsRight = new JPanel();
		layout = new BorderLayout();
		layout.setVgap(LAYOUT_GAP);
		ngramsRight.setLayout(layout);
		ngramsRight.add(ngramsOutput, BorderLayout.CENTER);

		JPanel ngrams = new JPanel();
		ngrams.setLayout(new BoxLayout(ngrams, BoxLayout.LINE_AXIS));
		ngrams.add(ngramsLeft);
		ngrams.add(ngramsRight);
		return ngrams;
	}
	private void ngramsTrain() {
	    String text = ngramsOutput.getText();
	    if(!text.equals("")) {
	    	Ngrams ngrams =	new Ngrams(text);
	    	System.out.println("ngrams result");
